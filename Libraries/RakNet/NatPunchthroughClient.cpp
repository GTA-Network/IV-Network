...
void NatPunchthroughClient::GetUPNPPortMappings(char *externalPort, char *internalPort, const SystemAddress &natPunchthroughServerAddress)
{
	...
	if (nextExternalPort==0)
		nextExternalPort=rakPeerInterface->GetExternalID(natPunchthroughServerAddress).GetPort();
	Itoa(nextExternalPort,externalPort,10);
}
...
void NatPunchthroughClient::OnGetMostRecentPort(Packet *packet)
{
	...
	if (nextExternalPort==0)
	{
		nextExternalPort=rakPeerInterface->GetExternalID(packet->systemAddress).GetPort();
		RakAssert(nextExternalPort!=0);
	}

	outgoingBs.Write(nextExternalPort);
