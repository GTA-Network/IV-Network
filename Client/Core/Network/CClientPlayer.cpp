...
CClientPlayer::CClientPlayer( bool bLocalPlayer )
{
...
unsigned short CClientPlayer::GetPing()
{
	if(IsLocalPlayer())
	{
...
void CClientPlayer::Serialize( CBitStream * bitStream )
{
...
void CClientPlayer::Deserialize( CBitStream * bitStream )
{
	// Is the player not spawned?
	if( !IsSpawned() )
		return;
...
void CClientPlayer::SetPosition(CVector3 vecPosition)
{
	if(IsSpawned())
...
bool CClientPlayer::GetPosition(CVector3 *vecPosition)
{
	if(IsSpawned())
...
void CClientPlayer::Teleport( CVector3 vecPosition )
{
...
void CClientPlayer::SetColor( unsigned int uiColor )
{
	// Save the colour
	m_uiColor = uiColor;
...
void CClientPlayer::SetHealth( float fHealth )
{
	m_pPlayerPed->SetHealth( fHealth );
}

float CClientPlayer::GetHealth()
{
	return (m_pPlayerPed->GetHealth());
}

void CClientPlayer::SetRotation( float fAngle )
{
...
float CClientPlayer::GetRotation()
{
	IVPed * pPed = m_pPlayerPed->GetPed();
...
void CClientPlayer::SetModel( int iModelId )
{
...
void CClientPlayer::SetControlState( CControls * pControlState )
{
	// Is the player spawned?
	if( IsSpawned() )
	{
...
void CClientPlayer::GetControlState( CControls * pControlState )
{
	// Copy the current controls
	memcpy( pControlState, &m_ControlState, sizeof(CControls) );
}

void CClientPlayer::GetLastControlState( CControls * pControlState )
{
	// Copy the last controls
	memcpy( pControlState, &m_lastControlState, sizeof(CControls) );
}

void CClientPlayer::InternalPutInVehicle( CClientVehicle * pVehicle, BYTE byteSeat )
{
...
void CClientPlayer::InternalRemoveFromVehicle( )
{
...
void CClientPlayer::PutInVehicle( CClientVehicle * pVehicle, BYTE byteSeat )
{
	// Are we not spawned?
	if( !IsSpawned() )
		return;
...
void CClientPlayer::RemoveFromVehicle( )
{
	// Are we not spawned?
	if( !IsSpawned() )
		return;
...
bool CClientPlayer::IsAnyWeaponUser()
{
...
void CClientPlayer::SendOnFootData( )
{
...
void CClientPlayer::SendInVehicleData( )
{
	// Construct a new bitstream
	CBitStream bitStream;
...
void CClientPlayer::SendPassengerData( )
{
	// Construct a new bitstream
	CBitStream bitStream;
...
void CClientPlayer::EnterVehicle( CClientVehicle * pVehicle, BYTE byteSeat )
{
	// Are we not spawned?
	if( !IsSpawned() )
		return;
...
void CClientPlayer::ExitVehicle( eExitVehicleType exitType )
{
	// Are we not spawned?
	if( !IsSpawned() )
		return;
...
void CClientPlayer::CheckVehicleEnterExit( )
{
	// Are we spawned?
	if( IsSpawned() )
	{
...
bool CClientPlayer::GetClosestVehicle( bool bPassenger, CClientVehicle ** pVehicle, BYTE& byteSeat )
{
	// Are we spawned?
	if( IsSpawned() )
	{
		...
		/*for( int i = 0; i < g_pClient->GetGame()->GetVehicleManager()->GetCount(); i++ )
		{
			...
			CClientVehicle * pThisVehicle = g_pClient->GetGame()->GetVehicleManager()->Get(i);
...
void CClientPlayer::ClearVehicleEntryTask( )
{
	// Are we spawned?
	if( IsSpawned() )
	{
...
void CClientPlayer::ClearVehicleExitTask( )
{
	// Are we spawned?
	if( IsSpawned() )
	{
...
void CClientPlayer::ProcessVehicleEnterExit( )
{
	// Are we spawned?
	if( IsSpawned() )
	{
